...
export const DiffPanel: React.FC<DiffPanelProps> = ({ delta }) => {
  ...
  return (
    <div className="mt-4 rounded-md border border-[var(--border)] bg-[var(--card)] p-3">
      ...
      <Section title="Stricter" items={delta.stricter || []} badgeClass="bg-red-500/10 text-red-500 border border-red-500/20" />
      <Section title="Looser" items={delta.looser || []} badgeClass="bg-green-500/10 text-green-500 border border-green-500/20" />
      <Section
        title="Additional Requirements"
        items={delta.additionalRequirements || []}
        badgeClass="bg-amber-500/10 text-amber-600 border border-amber-500/20"
      />
      <Section title="Exceptions" items={delta.exceptions || []} badgeClass="bg-indigo-500/10 text-indigo-500 border border-indigo-500/20" />
      <Section title="Replacements" items={delta.replacements || []} badgeClass="bg-slate-500/10 text-slate-500 border border-slate-500/20" />
      <Section
        title="Not Applicable"
        items={delta.notApplicable || []}
        badgeClass="bg-gray-500/10 text-gray-500 border border-gray-500/20"
      />
      <Section title="Additions" items={delta.additions || []} badgeClass="bg-blue-500/10 text-blue-500 border border-blue-500/20" />
    </div>
  );
};
