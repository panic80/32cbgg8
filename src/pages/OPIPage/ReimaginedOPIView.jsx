...
function EmailLink({ email, compact = false }) {
  if (!email) {
...
function ContactPanel({ title, contact, type }) {
  ...
  return (
    ...
    >
      <div className="mb-4 flex items-start justify-between gap-4">
        <div>
          <p className={cn('text-xs font-bold uppercase tracking-wide', style.emphasis)}>
            {title}
          </p>
          <h3 className="mt-1 text-xl font-semibold leading-tight text-foreground">{contact.name}</h3>
        </div>
        <span
          className={cn(
...
function ContactRow({ contact }) {
  ...
  const style = CONTACT_TYPE_STYLE[type] || CONTACT_TYPE_STYLE.FSC;
...
export default function ReimaginedOPIView({
  ...
  const [activeView, setActiveView] = useState(
    VIEW_OPTIONS.some((option) => option.id === initialView) ? initialView : 'all',
  );
