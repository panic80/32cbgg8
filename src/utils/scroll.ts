  ...
  try {
    ...
    for (const target of candidates) {
      ...
      if (typeof t.scrollTo === 'function') {
        try { t.scrollTo({ top: 0, left: 0, behavior: 'auto' }); } catch {}
      }
      if ('scrollTop' in t) {
        try { t.scrollTop = 0; } catch {}
      }
      if ('pageYOffset' in t && typeof t.pageYOffset === 'number') {
        try { t.pageYOffset = 0; } catch {}
      }
    }
  } catch {
    // no-op
...
export function forceScrollToTopDeferred(): () => void {
  ...
  raf2 = typeof window !== 'undefined' ? window.requestAnimationFrame(() => {
    setTimeout(forceScrollToTop, 10);
  }) : 0;
  
  // Third pass for content-heavy pages
  const t3 = setTimeout(forceScrollToTop, 200);
  raf3 = typeof window !== 'undefined' ? window.requestAnimationFrame(() => {
