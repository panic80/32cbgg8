...
const serializeError = (error) => {
    ...
    return Object.fromEntries(Object.entries(error).map(([key, value]) => [
        key,
...
class ChatLogger {
    ...
    emit(level, message, context) {
        ...
        const writer = resolvedLevel === 'error'
            ? console.error
            : resolvedLevel === 'warn'
                ? console.warn
                : console.log;
    ...
    child(binding = {}) {
        ...
        return {
            ...
            logVisit: (visitData) => this.logVisit(visitData),
            getVisitSummary: (filters) => this.getVisitSummary(filters),
            getChatLogs: (filters) => this.getChatLogs(filters),
    ...
    logChat(_req, chatData) {
        ...
        const askedAt = cd.timestamp ? Date.parse(cd.timestamp) : Date.now();
    ...
    logVisit(visitData = {}) {
        ...
        const additionalMetadata = visitData.metadata && typeof visitData.metadata === 'object' ? visitData.metadata : null;
    ...
    getChatLogs({ limit = 50, offset = 0, startAt, endAt, conversationId, model, provider, ragEnabled, shortAnswerMode, search, } = {}) {
        const conditions = [];
