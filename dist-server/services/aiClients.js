 ...
 */
export const isOSeriesModel = (model) => {
    return !!(model &&
 ...
 */
export const buildOpenAIParams = (model, messages) => {
    const baseParams = {
        model: model,
        messages: messages,
    };
...
const resolveGeminiApiKey = () => {
    ...
    if (isValidApiKey(process.env.VITE_GEMINI_API_KEY)) {
        logger.warn('VITE_GEMINI_API_KEY is deprecated. Migrate to GEMINI_API_KEY to keep credentials server-side.');
