...
export const isPrivateIpv6 = (ip) => {
    ...
    if (normalized.startsWith('fe80') || normalized.startsWith('fec0'))
...
export const validateIngestionUrl = async (rawUrl) => {
    if (!rawUrl || typeof rawUrl !== 'string') {
        const error = new Error('Ingestion URL is required');
    ...
    try {
        ...
        const err = new Error('Invalid ingestion URL format');
        err.statusCode = 400;
        throw err;
    }
    if (!['http:', 'https:'].includes(parsed.protocol)) {
        const error = new Error('Only HTTP and HTTPS ingestion URLs are allowed');
        error.statusCode = 400;
        throw error;
    }
    ...
    if (disallowedHostnames.has(hostname)) {
        const error = new Error('Ingestion URL may not target local addresses');
        error.statusCode = 400;
        throw error;
    }
    ...
    if (!Array.isArray(addresses) || addresses.length === 0) {
        const error = new Error('Unable to resolve ingestion URL host');
        error.statusCode = 400;
        throw error;
    }
    if (addresses.some(isAddressDisallowed)) {
        const error = new Error('Ingestion URL resolves to a private or disallowed address');
        error.statusCode = 400;
        throw error;
    }
